...
df_forward=pd.concat([dg.df_giocatori_seriea,dg.df_giocatori_premier])
#print(df_forward)
X=df_forward.iloc[:,[1,2,3,4,5,7]].values.reshape(-1,6)
y=df_forward.iloc[:,6].values.reshape(-1,1)
y=np.log(y)
"""
X=acv.X_four_std
y=acv.pts_s
...
clf = DecisionTreeRegressor(ccp_alpha=0.01,random_state=0)
...
tree.plot_tree(clf,fontsize=7,feature_names=["PPDA","OPPDA","DC","ODC"])
plt.show()
...
ax.legend()
plt.show()
### alpha=0.0074 returns for x_four facciamo 0.01 e tutti contenti
